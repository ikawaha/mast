...
func TestEq01(t *testing.T) {
	...
	crs := []struct {
		...
		{pair{&state{Output: map[byte]string{1: "go"}}, &state{Output: map[byte]string{1: "go"}}},
			true},
		{pair{&state{Output: map[byte]string{1: "go"}}, &state{Output: map[byte]string{1: "c++"}}},
			false},
		{pair{&state{Output: map[byte]string{1: "go"}}, &state{Output: map[byte]string{2: "go"}}},
			false},
		{pair{&state{Tail: map[string]bool{"go": true}}, &state{Tail: map[string]bool{"go": true}}}, true},
	}
	for _, cr := range crs {
